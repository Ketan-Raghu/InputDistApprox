"""
Stable L2L Training Pipeline — Multi-GPU hardware-accelerated version.

Like l2l_pipeline_fast.py but with a more conservative reinitialization:
  - Only the last 4 transformer layers (24-27) are reinitialized
  - The embedding matrix (embed_tokens) is kept pretrained
...
Usage:
  python l2l_pipeline_stable_fast.py [--batch-size 16] [--stage1-epochs 1] [--stage2-epochs 3]
"""
...
CHECKPOINT_DIR = os.path.join(os.path.dirname(__file__), "checkpoints", "l2l_stable_fast")
...
class FastL2LPipeline(nn.Module):
    ...
    def train_forward(self, prob_vectors, mask, temperature):
        ...
        # then let float32 softmax handle extreme-but-finite values — it
        # subtracts the max internally so no hard clamp is needed.  The old
        # clamp(-65504, 65504) had zero gradient at the boundary and was the
        # primary cause of vanishing grad_norm.
