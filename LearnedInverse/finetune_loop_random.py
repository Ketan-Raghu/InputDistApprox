...
def sample_random_logits(n, device, scale=10.0):
    """Sample random logit vectors resembling classifier outputs.

    Uses the rough scale of trained classifier logits (dominant class ~10,
    others near 0 or negative).
    """
    # Start from a uniform-ish base, then boost one random class
    base = torch.randn(n, 10, device=device) * 2.0
    dominant = torch.randint(0, 10, (n,), device=device)
    base[torch.arange(n, device=device), dominant] += scale
    return base


def generate_synthetic_dataset(inverse_model, classifier, n_samples, device, batch_size=512):
    ...
    with torch.no_grad():
        while remaining > 0:
            ...
            random_logits = sample_random_logits(bs, device)
...
def main():
    ...
    # Fine-tuning config
    outer_rounds = 10
    synth_per_round = 120000  # 2x real data -> 33% real / 67% synthetic
    finetune_epochs = 5
    batch_size = 64
    lr = 1e-4
    kl_weight = 1.0
    ...
    for round_idx in range(1, outer_rounds + 1):
        ...
        true_logits, synth_images = generate_synthetic_dataset(
            inverse_model, classifier, synth_per_round, device
        )
    ...
    save_path = os.path.join(script_dir, "inverse_ffnn_finetuned.pt")
    ...
    plt.tight_layout()
    plot_path = os.path.join(script_dir, "finetune_loop_samples.png")
