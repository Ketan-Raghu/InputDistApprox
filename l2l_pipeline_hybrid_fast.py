"""
Hybrid L2L Training Pipeline — Multi-GPU hardware-accelerated version.

Based on l2l_pipeline_stable_fast.py with a stage-dependent middle model
output strategy:

  Stage 1: Regular softmax on middle model logits (temperature-scaled).
           Smooth gradients while the reinitialized layers stabilize.
  Stage 2: Gumbel-Softmax (hard=True, straight-through estimator).
           As temperature decays toward 0, the model transitions to
           discrete token selection — Model B sees clean one-hot
           embeddings in the forward pass while gradients still flow
           through the soft Gumbel approximation in the backward pass.
...
Usage:
  python l2l_pipeline_hybrid_fast.py [--batch-size 16] [--stage1-epochs 1] [--stage2-epochs 3]
"""
...
CHECKPOINT_DIR = os.path.join(os.path.dirname(__file__), "checkpoints", "l2l_hybrid_fast")
...
class TrainingConfig:
    ...
    stage1_temp_start: float = 2.0
    stage1_temp_end: float = 1.0
    stage1_temp_warmup_steps: int = 500
    stage2_temp_start: float = 0.5
...
class FastL2LPipeline(nn.Module):
    ...
    Three separate model instances avoid device_map="auto" fragmentation:
      ...
      Middle  (GPU 1) — trainable, reinitialized layers 24-27 (embed_tokens kept pretrained)
    ...
    def _register_grad_hooks(self):
        """Register backward hooks on every transformer layer to track
        gradient flow through the middle model and Model B."""
        self._layer_grad_norms = {}

        def make_hook(name):
            def hook(module, grad_input, grad_output):
                g = grad_output[0] if isinstance(grad_output, tuple) else grad_output
                if g is not None:
                    self._layer_grad_norms[name] = g.float().norm().item()
            return hook

        mid = self._get_saveable_middle()
        mid.model.embed_tokens.register_full_backward_hook(make_hook("mid_embed"))
        for i, layer in enumerate(mid.model.layers):
            layer.register_full_backward_hook(make_hook(f"mid_{i}"))

        for i, layer in enumerate(self.model_b.model.layers):
            layer.register_full_backward_hook(make_hook(f"b_{i}"))

    def plot_layer_grad_norms(self, step, save_dir):
        """Save a bar chart of per-layer gradient norms to save_dir."""
        if not self._layer_grad_norms:
            return

        # Build ordered labels: mid_embed, mid_0..27, b_0..27
        labels, values = [], []
        for key in ["mid_embed"] + [f"mid_{i}" for i in range(NUM_LAYERS)] + [f"b_{i}" for i in range(NUM_LAYERS)]:
            if key in self._layer_grad_norms:
                labels.append(key)
                values.append(self._layer_grad_norms[key])

        values = np.array(values)
        colors = ["#2196F3" if l.startswith("mid_") else "#FF9800" for l in labels]
        # Highlight reinit layers
        reinit_start = NUM_LAYERS - NUM_REINIT_LAYERS
        for i, l in enumerate(labels):
            if l.startswith("mid_") and l != "mid_embed":
                idx = int(l.split("_")[1])
                if idx >= reinit_start:
                    colors[i] = "#F44336"
        ...
        ax.set_yscale("log")
        ax.set_ylim(bottom=max(values[values > 0].min() * 0.1, 1e-12) if (values > 0).any() else 1e-12)
        ax.set_xticks(x)
        ...
        ax.legend(handles=[
            Patch(color="#2196F3", label="Middle (pretrained)"),
            Patch(color="#F44336", label="Middle (reinitialized)"),
            Patch(color="#FF9800", label="Model B (frozen)"),
        ], loc="upper right", fontsize=8)

        fig.tight_layout()
        plot_dir = os.path.join(save_dir, "grad_norm_plots")
        os.makedirs(plot_dir, exist_ok=True)
        fig.savefig(os.path.join(plot_dir, f"layer_grad_norms_step{step:06d}.png"), dpi=150)
        plt.close(fig)

    def _reinit_middle(self):
        start = NUM_LAYERS - NUM_REINIT_LAYERS
        print(f"  Reinitializing layers {start}-{NUM_LAYERS - 1} (embed_tokens kept pretrained)")
    ...
    def configure_stage1(self):
        """Stage 1: only train reinitialized params (layers 24-27)."""
    ...
    def train_forward(self, prob_vectors, mask, temperature, use_gumbel=False):
        """
        ...
        temperature is applied to the middle model's output logits (matching
        the temperature used on Model A's input) so the softmax stays soft
        and gradients can flow — same principle as knowledge distillation.

        use_gumbel: if True, use Gumbel-Softmax (straight-through) instead
        of regular softmax for the middle model's output. This gives Model B
        clean one-hot embeddings in the forward pass while gradients still
        flow through the soft approximation in the backward pass.
        """
        # -- Middle model (GPU 1) --
        ...
        # When embed_tokens is frozen (Stage 1), soft_embeds has no grad.
        # Gradient checkpointing requires at least one input with grad so
        # that the backward pass can reach the trainable layers (24-27).
        if not soft_embeds.requires_grad:
        ...
        # Sanitize NaN/Inf in logits (reinitialized layers can produce them),
        # then let float32 softmax handle extreme-but-finite values.
        middle_logits = middle_out.logits.float()
        middle_logits = torch.nan_to_num(
            middle_logits, nan=0.0, posinf=65504.0, neginf=-65504.0,
        )
        temp = max(temperature, 1e-6)

        if use_gumbel and self.training:
            # Gumbel-Softmax with straight-through: one-hot forward, soft
            # backward.  Model B sees clean discrete embeddings while
            # gradients flow through the soft Gumbel approximation.
            B, S, V = middle_logits.shape
            middle_probs = F.gumbel_softmax(
                middle_logits.view(-1, V), tau=temp,
                hard=self.config.gumbel_hard,
            ).view(B, S, V).to(self.dtype)
        else:
            # Regular temperature-scaled softmax (Stage 1 / validation).
            middle_probs = F.softmax(middle_logits / temp, dim=-1).to(self.dtype)
        ...
        log_probs = F.log_softmax(b_logits.float(), dim=-1).clamp(min=-100.0)
        per_token_loss = -(targets.float() * log_probs).sum(dim=-1)
    ...
    def forward(self, input_ids, attention_mask, temperature):
        ...
        return self.train_forward(prob_vectors, mask, temperature)
...
def train_one_epoch(
    ...
    *, stage, epoch, global_step, temperature_fn, log_path, config,
    ...
    for batch_idx, (prob_vectors, mask) in pbar:
        ...
        loss = pipeline.train_forward(prob_vectors, mask, temp, use_gumbel=use_gumbel)
        ...
        if (batch_idx + 1) % config.gradient_accumulation_steps == 0:
            ...
            if global_step % config.log_every == 0:
                ...
                pipeline.plot_layer_grad_norms(global_step, config.checkpoint_dir)
...
def validate(pipeline, dataloader, temperature, max_batches=50):
    ...
    NOTE: we intentionally do NOT call pipeline.middle_model.eval() here.
    Toggling eval/train on a model with gradient_checkpointing enabled
    corrupts the checkpointing state, causing gradients to silently become
    zero when training resumes.  torch.no_grad() (from the decorator) is
    sufficient to disable gradient computation during validation.
    """
    total_loss = 0.0
    count = 0
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Validating", leave=False)):
        ...
        loss = pipeline.train_forward(prob_vectors, mask, temperature)
...
def main():
    ...
    parser.add_argument("--stage1-temp-start", type=float, default=2.0)
    parser.add_argument("--stage1-temp-end", type=float, default=1.0)
    parser.add_argument("--stage1-temp-warmup-steps", type=int, default=500)
    ...
    parser.add_argument("--gumbel-hard", action="store_true", default=True,
                        help="Stage 2: use straight-through Gumbel-Softmax (one-hot fwd, soft bwd)")
    parser.add_argument("--no-gumbel-hard", dest="gumbel_hard", action="store_false",
                        help="Stage 2: use soft Gumbel-Softmax (no straight-through)")
    ...
    config = TrainingConfig(
        ...
        stage1_temp_start=args.stage1_temp_start,
        stage1_temp_end=args.stage1_temp_end,
        stage1_temp_warmup_steps=args.stage1_temp_warmup_steps,
    ...
    # -- Resume handling ----------------------------------------------------
    # 0 = start fresh, 1 = stage 1 done, 2 = stage 2 mid
    resume_stage = 0
    resume_step = 0
    resume_epoch = 0
    if args.resume:
    ...
    if resume_stage < 1:
        print(f"\n{'=' * 60}")
        print(f"STAGE 1: Train reinitialized layers only "
              f"(temp {config.stage1_temp_start:.1f} -> {config.stage1_temp_end:.1f})")
        print(f"{'=' * 60}")
        ...
        def temp_fn_s1(step):
            if step < config.stage1_temp_warmup_steps:
                return config.stage1_temp_start
            decay_step = step - config.stage1_temp_warmup_steps
            decay_total = total_s1 - config.stage1_temp_warmup_steps
            return get_temperature(
                decay_step, decay_total, config.temp_schedule,
                config.stage1_temp_start, config.stage1_temp_end,
            )
        ...
        for epoch in range(config.stage1_epochs):
            global_step = train_one_epoch(
                ...
                temperature_fn=temp_fn_s1,
                log_path=log_path, config=config,
            )
            cur_temp = temp_fn_s1(global_step)
            val_loss = validate(pipeline, val_loader, cur_temp)
            print(f"  Stage 1 Epoch {epoch} val_loss: {val_loss:.4f}, temp: {cur_temp:.4f}")
            with open(log_path, "a") as f:
                f.write(json.dumps({
    ...
    # -- Stage 2 ------------------------------------------------------------
    print(f"\n{'=' * 60}")
    print("STAGE 2: Full finetune + Gumbel-Softmax temperature annealing")
    print(f"{'=' * 60}")
