...
def main():
    ...
    model.load_state_dict(
        torch.load(os.path.join(LEARNED_DIR, "inverse_ffnn.pt"), map_location=device, weights_only=True)
    )
    model.eval()
    ...
    fig.suptitle("Inverse FFNN Per-Digit Distribution (mean +/- std)", fontsize=14, y=1.02)
    plt.tight_layout()
    out_path = os.path.join(SCRIPT_DIR, "inverse_per_digit_distribution.png")
