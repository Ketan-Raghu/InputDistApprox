"""
Gumbel-Softmax L2L Training Pipeline — Multi-GPU hardware-accelerated version.

Based on l2l_pipeline_stable_fast.py.  The key difference is how the middle
model's output distribution is converted to embeddings for Model B:

  stable_fast:  weighted_embeds = softmax(logits / T)  @  embed_cache
  gumbel_fast:  weighted_embeds = gumbel_softmax(logits, tau=T, hard=True)  @  embed_cache

With hard=True (straight-through Gumbel-Softmax):
  - Forward: one-hot argmax → Model B sees clean discrete token embeddings
    (the kind it was pretrained on), not blurry weighted averages.
  - Backward: gradients flow through the soft Gumbel-Softmax approximation,
    so the middle model's layers still receive learning signal.
  - Gumbel noise adds stochastic exploration, helping escape mode collapse.
...
Usage:
  python l2l_pipeline_gumbel_fast.py [--batch-size 16] [--stage1-epochs 1] [--stage2-epochs 3]
"""
...
CHECKPOINT_DIR = os.path.join(os.path.dirname(__file__), "checkpoints", "l2l_gumbel_fast")
...
class FastL2LPipeline(nn.Module):
    ...
    def train_forward(self, prob_vectors, mask, temperature):
        """
        ...
        if self.training:
            # Gumbel-Softmax: adds Gumbel noise for stochastic exploration.
            # hard=True uses straight-through (one-hot forward, soft backward)
            # so Model B sees clean discrete embeddings while gradients still
            # flow through the soft approximation.
            # F.gumbel_softmax takes raw logits and tau (temperature) directly.
            ...
            # Deterministic for validation — no Gumbel noise.
...
def train_one_epoch(
    ...
    *, stage, epoch, global_step, temperature_fn, log_path, config,
    ...
    for batch_idx, (prob_vectors, mask) in pbar:
        ...
        loss = pipeline.train_forward(prob_vectors, mask, temp)
...
def main():
    ...
    parser.add_argument("--gumbel-hard", action="store_true", default=True,
                        help="Use straight-through Gumbel-Softmax (one-hot forward, soft backward)")
    parser.add_argument("--no-gumbel-hard", dest="gumbel_hard", action="store_false",
                        help="Use soft Gumbel-Softmax (no straight-through)")
